...
public class AuthCodeToCookieHandler implements RequestHandler<Map<String, ?>, LambdaResponse> {
    ...
    public LambdaResponse handleRequest(Map<String, ?> input, Context context) {
        ...
        try {
            ...
            if (postBody.isPresent()) {
                ...
                if (code.isPresent() && codeVerifier.isPresent()) {
                    var tokens = exchanger.exchangeAuthCodeForTokens(code.get(), codeVerifier.get());
                    if (!tokens.hasError()) {
                        ...
                        if (email.isPresent()) {
                            ...
                            return LambdaResponse.builder().statusCode(400).body(Map.of("userEmail", "No email!")).build();
                        }
                    }
                    return LambdaResponse.builder().statusCode(400).body(Map.of("message", "Invalid Code")).build();
        ...
        return LambdaResponse.builder().statusCode(400).body(Map.of("message", "Something went wrong!")).build();
    }
    ...
    private Optional<AuthCodeToCookieParams> parseBody(LambdaLogger logger, Map<String, ?> input) throws JsonProcessingException {
    ...
    private Optional<String> readCode(LambdaLogger logger, AuthCodeToCookieParams params) throws JsonProcessingException {
        if (params.code() == null) {
    ...
    private Optional<String> readCodeVerifier(LambdaLogger logger, AuthCodeToCookieParams params) throws JsonProcessingException {
